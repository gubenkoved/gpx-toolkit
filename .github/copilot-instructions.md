...
| [src/windspeed-view.ts](../src/windspeed-view.ts) | Wind/Speed view (`#analyticsView`): a confirm-to-run gate (centred card naming how many rides the window will analyse, live as the slider moves), the window-scoped per-ride segment sweep (cached), distance-weighted regression, the scatter + KPI cards + empty/blocked states. Behind a `WindSpeedDeps` seam | `initWindSpeedView()`, `mountWindSpeedView()`, `windSpeedVisibleRides()`, `WindSpeedDeps` |
